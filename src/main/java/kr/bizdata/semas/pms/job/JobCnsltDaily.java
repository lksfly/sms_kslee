 ...
 */
@Component
public class JobCnsltDaily {
	
	private final Logger logger = LoggerFactory.getLogger(getClass());
	
	@Autowired
	private FromCnsltService fromService;
	@Autowired
	...
	public JobResult run(String jobNm) {
		...
		{
			List<FromCnsltInfo> list = fromService.selectList(startDt, endDt, "1"); // 승인
			for (FromCnsltInfo from : list) {
				try {
			...
			List<FromCnsltInfo> list = fromService.selectList(startDt, endDt, "2"); // 취소
			for (FromCnsltInfo from : list) {
				try {
					jobProcessService.process(from); ++sCnt; // 데이터 처리
				} catch (Exception e) {

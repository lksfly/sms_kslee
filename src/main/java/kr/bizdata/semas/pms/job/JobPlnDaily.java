 ...
 */
@Component
public class JobPlnDaily {
	
	private final Logger logger = LoggerFactory.getLogger(getClass());
	
	@Autowired
	private FromPlnService fromService;
	@Autowired
	...
	public JobResult run(String jobNm) {
		...
		int eCnt = 0; // 오류수
		{
			List<FromPlnInfo> list = fromService.selectList(startDt, endDt, "1", isProdUse); // 대리대출 승인
			for (FromPlnInfo from : list) {
				try {
